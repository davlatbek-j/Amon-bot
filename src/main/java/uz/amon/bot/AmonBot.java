    ...
    private void patientMessageHandler(Update update) throws TelegramApiException, IOException
    {
        ...
        switch (state)
        {
            ...
            case FIRST_NAME:
                ...
                break;
            case LAST_NAME:
                patientLastNameHandler(update); //shu yerda kerakli doktorni ham tanlaydi
                break;
            case PHOTO:
...
//        keyboard.add(row2);
        ...
        ReplyKeyboardMarkup markup2 = new ReplyKeyboardMarkup();
        List<KeyboardRow> rowList = new ArrayList<>();
        KeyboardButton button = new KeyboardButton();
        markup2.setResizeKeyboard(true);
        if (patient.getLanguage().equals(PatientLanguage.UZ))
            button.setText("Yangi murojaat qoldirish");
        else if (patient.getLanguage().equals(PatientLanguage.RU))
