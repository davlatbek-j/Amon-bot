...
//    Method name, lastname, surname uchun validatsiya. textName ga kirib kelishi mumkin(name, lastname, surname).
//    textga esa patient yozgan text kirib keladi.
    public String validateNames(String text, String textName) {
        if (!text.matches("^[A-Za-z]+$") || text.length() < 2 || text.length() > 50) {
            return textName + " faqat harflardan iborat bo'lishi kerak, va uzunligi " +
                    "2 dan 50 gacha bo'lishi kerak.";
        }
